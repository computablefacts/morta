...
public abstract class DocSetLabeler {
  ...
  public List<Map.Entry<String, Double>> label(List<String> corpus, List<String> subsetOk,
    ...
    AtomicInteger nbTexts = new AtomicInteger(corpus.size());

    corpus.parallelStream().forEach(text -> {

      nbTextsProcessed.incrementAndGet();

      if (nbTextsProcessed.get() % 100 == 0) {
        System.out.printf("%d%% of all texts processed...%n",
            nbTextsProcessed.get() * 100 / nbTexts.get());
      }

      Queue<Map.Entry<String, Double>> weights = new ConcurrentLinkedQueue<>();
